  ...
  <body class="hold-transition skin-blue sidebar-mini">
                    ...
                    <div class="artikel-list-body col-xs-12 no-padding">
                        <div class="col-xs-4 left-list-body">
                            <div>
                                <div class="artikel-img-preview-wrap">
                                  <img src="<?php echo base_url('').$kedai[0]['LOGO_KEDAI'] ?>" class="img-responsive" id="imgPreview">
                        ...
                        <div class="col-xs-8 right-list-body">
                            <div class="input-group">
                                <span class="input-group-btn">
                                    <span class="btn btn-default">ID Kedai</span>
                                  </span>
                                <input type="text" class="form-control" name="id" value="<?php echo $kedai[0]['ID_KEDAI'] ?>" readonly>
                            </div>
                            <div class="input-group">
                                <span class="input-group-btn">
                                    <span class="btn btn-default">Nama Kedai</span>
                                  </span>
                                <input type="text" class="form-control" name="nama" value="<?php echo $kedai[0]['NAMA_KEDAI'] ?>" required>
                            </div>
                            <div class="input-group">
                                <span class="input-group-btn">
                                    <span class="btn btn-default">Admin Kedai</span>
                                  </span>
                                <input type="text" class="form-control" name="admin" value="<?php echo $kedai[0]['ADMIN_KEDAI'] ?>" required>
                            </div>
                        </div>
                        <div class="col-xs-12">
                            <div id="submitWrap">
    ...
    $('#file').change(function(){
        ...
        $("#gambar-artikel").submit(function(e){
            ...
            $.ajax({
                type : 'POST',
                url : '<?php echo site_url('admin/gambar/kedai')?>',
    ...
    $("#formCropGambar").submit(function(e){
        ...
        var imageName = $('#imgReadyCrop').attr('src').split(/(\\|\/)/g).pop();
          $.ajax('<?php echo site_url('admin/gambarCrop/kedai')?>', {
            method: "POST",
    ...
    $(document).ready(function(){
        ...
        $("#form-artikel").submit(function(event){
            ...
            $.ajax({
                type : 'POST',
                url : '<?php echo site_url('admin/kedai_edit')?>',
                data : formData,
                cache: false,
                contentType: false,
                processData: false,
                ...
                success : function(berhasil){
                    // alert(berhasil);
                    swal('Berhasil Mengedit Kedai','','success').then(function() {
                        window.location='<?php echo site_url('admin/kedai') ?>';
                    });
                }
            });
        });
